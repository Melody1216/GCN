...
parser.add_argument('--eval_epochs', default=50, type=int,
                    help="Evaluation epochs")
...
def run(num_epochs):
    ...
    for ep in range(num_epochs):
        if ep > num_epochs - args.eval_epochs:
            print('Evaluation：')
            policy_net.eval()
            start_eval = True
        ...
        for n in range(args.epoch_size):
            ...
            if start_eval:
                _, s = trainer.run_batch(ep, is_eval=start_eval)
            else:
        ...
        if float(stat['add_rate']) >= 0.05:
            success_record.append([stat['success'], np.mean(stat['reward'])])
        if args.save != '' and float(stat['add_rate']) >= 0.05 and float(stat['success']) > pre_success:
            save('models/Success_{}.pkl'.format(int(round(stat['success'], 2) * 100)))
            pre_success = float(stat['success'])

        gc.collect()
    column = ['success', 'reward']
    data_log = pd.DataFrame(columns=column, data=success_record)
    data_log.to_csv('log.csv')

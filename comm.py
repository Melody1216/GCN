...
class CommNetMLP(nn.Module):
    ...
    def __init__(self, args, num_inputs):
        ...
        self.communication_mean = 0
        self.communication_var = 0
    ...
    def forward(self, x, info={}):
        ...
        for i in range(self.comm_passes):
            ...
            if info['is_eval']:
                comm_sum = (comm_sum - info['mean']) / (info['var'] + 0.0000000000001)
            c = self.C_modules[i](comm_sum)
            # c = self.bandwidth_limit(c)
        ...
        if self.args.recurrent:
            return action, value_head, (hidden_state.clone(), cell_state.clone()), c, agent_mask_1_dim
        else:
            return action, value_head, comm_sum, agent_mask_1_dim
